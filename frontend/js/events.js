// Sample data for ID cards (unchanged)
const idCards = [
  {
    id: 1,
    name: "Student 1",
...
// Modified sample data for trust schemes with scalable rc1.jpg to rc12.jpg images
const trustSchemes = Array.from({ length: 15 }, (_, i) => {
    ...
    return {
        ...
        description: `This trust scheme provides financial assistance for ${["education", "healthcare", "sports", "arts"][Math.floor(Math.random() * 4)]} related expenses.`,
    ...
    return `
        <div class="card fade-in" data-category="id-cards">
            ...
            <div class="card-content">
                ...
                ${!isChild ? '<button class="action-btn view-details-btn">View Details</button>' : ''}
            </div>
        </div>
    `;
}
...
function createTrustSchemeCard(scheme, isChild = false) {
    return `
        <div class="card fade-in" data-category="trust-schemes">
            <div class="card-image">
                <img src="${scheme.image}" alt="${scheme.title}">
                <span class="card-tag">Trust Scheme</span>
            </div>
            <div class="card-content">
            </div>
        </div>
...
// ID Cards expansion functionality (unchanged)
...
// Trust Schemes expansion functionality (unchanged)
